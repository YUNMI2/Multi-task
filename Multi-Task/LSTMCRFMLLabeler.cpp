...
void Labeler::train(const string& trainFiles, const string& trainCorpusSize, const string& trainCorpusTags, const string& devFiles, const string& testFiles, const string& testCorpusTags, const string& modelFile, const string& optionFile) {
	/*
	...
	*/
	...
	vector<string> vecTrainFiles, vecDevFiles, vecTestFiles, vecTrainCorpusSize, vecTrainCorpusTags, vecTestCorpusTags;
	split_bychar(trainFiles, vecTrainFiles, ',');
	split_bychar(trainCorpusSize, vecTrainCorpusSize, ',');
    split_bychar(trainCorpusTags, vecTrainCorpusTags, ',');
	assert(vecTrainFiles.size() == vecTrainCorpusSize.size() && vecTrainCorpusTags.size() == vecTrainCorpusSize.size());

	split_bychar(devFiles, vecDevFiles, ',');
	split_bychar(testFiles, vecTestFiles, ',');
    split_bychar(testCorpusTags, vecTestCorpusTags, ',');
 	assert(vecDevFiles.size() == vecTestFiles.size() && vecDevFiles.size() == vecTestCorpusTags.size());
	...
	// set parameter
  	...
  	m_classifier.init(wordEmb, m_options.wordcontext, tagEmbs, m_labelAlphabet.size(), m_options.rnnHiddenSize, m_options.hiddenSize, vecTrainCorpusTags.size());
    ...
    //		batchBlock++;
    /*
    vector<int> batchBlocks;
    for (int i = 0; i < vecSampleSize.size(); i++){
        int batchBlock = vecSampleSize[i] / m_options.batchSize;
  	    if (vecSampleSize[i] % m_options.batchSize != 0)
        	batchBlock++;
        batchBlocks.push_back(batchBlock);
  	...
  	for (int iter = 0, stopIter = 0; iter < m_options.maxIter  && stopIter < m_options.stopIter; ++iter, ++stopIter) {
		...
		for(int i = 0; i < vecSampleSize.size(); i++){
	        int batchBlock = vecSampleSize[i] / m_options.batchSize;
  	        if (inputSize % m_options.batchSize != 0)
                batchBlock++;
		    //random_shuffle(indexesAll.begin(), indexesAll.end());因为是Multi-Task，只是内部打乱，不全局打乱，要不然一个batch内部就要区分属于哪个语料
			...
			m_pipe.initInputFile(vecTrainFiles[i].c_str());
			for (int i_size = 0; i_size < vecSampleSize[i]; i_size++){
				posi = vecTrainInstsPositions[i][indexesTrains[i][i_size]];
			    Instance *pInstance = m_pipe.readInstanceByPosi(posi);
		    ...
		    assert(allSampleTrainInstances.size() == vecSampleSize[i]);
    	    ...
    	    for (int updateIter = 0; updateIter < batchBlock; updateIter++) {
      		    ...
      		    for (int idy = start_pos; idy < end_pos; idy++) {
        		    subExamples.push_back(allSampleTrainExamples[idy]);
      		    }

      		    int curUpdateIter = iter * batchBlock + updateIter;
      		    dtype cost = m_classifier.process(subExamples, curUpdateIter, i);
		...
		for (int i_dev = 0; i_dev < vecDevFiles.size(); i_dev++){
    		if (vecDevExamples[i_dev].size() > 0) {
                int corpusIndex = -1;
                for (int i = 0; i < vecTrainCorpusTags.size(); i++){
                    if (vecTrainCorpusTags[i].compare(vecTrainCorpusTags[i_dev]) == 0){
                        corpusIndex = i;
                        break;
                    }
                }
                assert(corpusIndex > -1);
      			...
      			for (int idx = 0; idx < vecDevExamples[i_dev].size(); idx++) {
        			...
        			predict(vecDevExamples[i_dev][idx].m_features, result_labels, vecDevInsts[i_dev][idx].words, corpusIndex);
				...
				std::cout << vecDevFiles[i_dev] << " Dev:" << std::endl;
      			...
      			if (vecTestExamples[i_dev].size() > 0) {
                    ...
                    assert(vecTestCorpusTags[i_dev].compare(vecTrainCorpusTags[corpusIndex]) == 0);
        			for (int idx = 0; idx < vecTestExamples[i_dev].size(); idx++) {
          				vector<string> result_labels;
          				predict(vecTestExamples[i_dev][idx].m_features, result_labels, vecTestInsts[i_dev][idx].words, corpusIndex);
...
int Labeler::predict(const vector<Feature>& features, vector<string>& outputs, const vector<string>& words, int corpusIndex) {
  assert(features.size() == words.size());
  vector<int> labelIdx, label2Idx;
  m_classifier.predict(features, labelIdx, corpusIndex);
  outputs.clear();
...
int main(int argc, char* argv[]) {
	...
	std::string trainFiles = "", trainCorpusSize = "", trainCorpusTags = ""; 
    std::string devFiles = "", testFiles = "", testCorpusTags = ""; 
	...
	ah.new_named_string("trainTags", "trainCorpusTags", "named_string",
			"training corpus kinds to train a model, must when training", trainCorpusTags);
	...
	ah.new_named_string("testTags", "testCorpusTags", "named_string",
			"testing corpus kinds to test a model, optional when training and must when testing", testCorpusTags);
	...
	Labeler tagger;
	if (bTrain) {
		tagger.train(trainFiles, trainCorpusSize, trainCorpusTags, devFiles, testFiles, testCorpusTags, modelFile, optionFile);
	} else {
	}
