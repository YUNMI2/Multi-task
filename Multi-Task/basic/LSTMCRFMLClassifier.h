...
public:
    ...
    vector<UniLayer<xpu> > _olayer_linears;
  	vector<UniLayer<xpu> > _tanh_projects;
  	LSTM<xpu> rnn_left_project;
  	LSTM<xpu> rnn_right_project;
  	vector<MLCRFLoss<xpu> > _crf_layers;
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, int wordcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize, int lstmhiddensize, int hiddensize, int mulTaskNum) {
   	...
   	_wordwindow = 2 * _wordcontext + 1;
        ...
        _tanh_projects.resize(mulTaskNum);
        _olayer_linears.resize(mulTaskNum);
        _crf_layers.resize(mulTaskNum);
        
        for(int i = 0; i < mulTaskNum; i++){
    	    _tanh_projects[i].initial(_hiddensize, 2 * _lstmhiddensize, true, 55, 0);
        	_olayer_linears[i].initial(_labelSize, _hiddensize, false, 60, 2);
    	    _crf_layers[i].initial(_labelSize, 70);
        }
  ...
  inline void release() {
   	_words.release();
        ...
        for(int i = 0; i < _olayer_linears.size(); i++){
    	    _olayer_linears[i].release();
    	    _crf_layers[i].release();
    	    _tanh_projects[i].release();
        }
        _olayer_linears.clear();
        _crf_layers.clear();
        _tanh_projects.clear();
    	rnn_left_project.release();
    	rnn_right_project.release();

  }

  inline dtype process(const vector<Example>& examples, int iter, const int corpusIndex) {
    	_eval.reset();
    	...
    	for (int count = 0; count < example_num; count++) {
         ...
         // concat tag input
      		...
      		_tanh_projects[corpusIndex].ComputeForwardScore(lstmoutput, project);



      		_olayer_linears[corpusIndex].ComputeForwardScore(project, output);

      		// get delta for each output
      		cost += _crf_layers[corpusIndex].loss(output, example.m_labels, outputLoss, _eval, example_num);

      		// loss backward propagation
      		// output
      		_olayer_linears[corpusIndex].ComputeBackwardLoss(project, output, outputLoss, projectLoss);

      		_tanh_projects[corpusIndex].ComputeBackwardLoss(lstmoutput, project, projectLoss, lstmoutputLoss);
      		for (int idx = 0; idx < seq_size; idx++) {
  ...
  void predict(const vector<Feature>& features, vector<int>& results, const int corpusIndex) {
    ...
    // decode algorithm
    _crf_layers[corpusIndex].predict(output, results);
  ...
  dtype computeScore(const Example& example, const int corpusIndex) {
    int seq_size = example.m_features.size();
    int offset = 0;
    ...
    // get delta for each output
    dtype cost = _crf_layers[corpusIndex].cost(output, example.m_labels);
  ...
  void updateParams(dtype belta1, dtype belta2, dtype nnRegular, dtype adaAlpha, dtype adaEps){
    ...
    for (int i = 0; i < _olayer_linears.size(); i++){
        _tanh_projects[i].updateAdam(belta1, belta2, nnRegular, adaAlpha, adaEps);
        _olayer_linears[i].updateAdam(belta1, belta2, nnRegular, adaAlpha, adaEps);
        _crf_layers[i].updateAdam(belta1, belta2, nnRegular, adaAlpha, adaEps);
    }
